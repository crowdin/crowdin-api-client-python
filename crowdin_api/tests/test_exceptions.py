...
from crowdin_api.exceptions import CrowdinException, APIException
...
class TestAPIException:
    ...
    def test_init(self, status_code, should_retry, result):
        detail = "some detail"
        exception = APIException(detail=detail, http_status=status_code, should_retry=should_retry)
    ...
    def test_message(self, m_template):
        ...
        assert (
            APIException(http_status=1, headers={"request-id": 2}, detail=3).message == "template"
        )
        ...
        template.format.called_once_with(1, 2, 3)
